...
// ------------------ Globale Variablen ---------------------
// Die gelesene Zeit...
int Brightness, clockInfoColor = 0, hours, minutes, Moonphase, seconds, WeatherTemperatur, WeatherHumidity, ldrbright;
...
Syslog syslog(udpClient, SYSLOGSERVER_SERVER, SYSLOGSERVER_PORT, "ESP8266", "LiquidClock", LOG_KERN);
#endif SYSLOGSERVER
...
void setup() {
  ...
  SyslogText("info" , "Neo-Pixel-Liquid-Clock-WEMOS");
  strip.begin();
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    SyslogText("info" , "Starting OTA service.");


    ArduinoOTA.onStart([]()
    {
      Serial.println("Start OTA update.");
      SyslogText("info" , "Start OTA update.");
    });
    ArduinoOTA.onError([](ota_error_t error)
    {
      Serial.println("OTA Error: " + String(error));
      SyslogText("info" , "OTA Error: " + String(error));
      if (error == OTA_AUTH_ERROR) {
        Serial.println("Auth failed.");
        SyslogText("error" , "Auth failed.");
      }
      else if (error == OTA_BEGIN_ERROR) {
        Serial.println("Begin failed.");
        SyslogText("error" , "Begin failed.");
      }
      else if (error == OTA_CONNECT_ERROR) {
        Serial.println("Connect failed.");
        SyslogText("error" , "Connect failed.");
      }
      else if (error == OTA_RECEIVE_ERROR) {
        Serial.println("Receive failed.");
        SyslogText("error" , "Receive failed.");
      }
      else if (error == OTA_END_ERROR) {
        Serial.println("End failed.");
        SyslogText("error" , "End failed.");
      }
    });
    ArduinoOTA.onEnd([]()
    {
      Serial.println("End OTA update.");
      SyslogText("info" , "End OTA update.");
    });
...
{ 
  ...
  if (modus == "clock" || modus == "clockInfo" || modus == "clockInfo2")
  {
    ...
    if (ldrbright > settings.getBrightness()) {
...
void SyslogText(String type , String text)
{
#ifdef SYSLOGSERVER
  if (type = "info") syslog.log(LOG_INFO, text);
  if (type = "error") syslog.log(LOG_ERR, text);
#endif SYSLOGSERVER
}
...
void wlan(bool an) {
 ...
 if(settings.getwlan())
 {
  Serial.println("Schalte Wlan ab");
    WiFi.mode(WIFI_OFF);
 }
  }
...
void setupWebServer()
{
  ...
  esp8266WebServer.on("/commitSettings", []() {
    handleCommitSettings();
  });//; callBack();});
  esp8266WebServer.on("/modus", []() {
    handleModus();
  });//; callBack();});
  esp8266WebServer.on("/moon", []() {
    handleMoon();
  });//; callBack();});
  esp8266WebServer.on("/temp", []() {
    handleTemp();
  });//; callBack();});
  esp8266WebServer.on("/brightness", []() {
    handleBrightness();
  });//; callBack();});
